...
function compareValue(operator, fieldValue, expected) {
    switch (operator) {
        case "equals": {
            if (fieldValue === expected)
                return true;
            if (normalizeBool(fieldValue) === normalizeBool(expected))
                return true;
        ...
        case "in": {
            if (!Array.isArray(expected))
                return false;
            return expected.some((e) => String(e) === String(fieldValue ?? "") ||
...
function evaluateCondition(rule, attributes) {
    if (!rule || !rule.field)
        return true;
